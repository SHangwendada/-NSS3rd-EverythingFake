...
//    uint32_t needed_count = 0;
    for (ElfW(Dyn)* d = dynamic; d->d_tag != DT_NULL; ++d) {
       // LOGD("d = %p, d[0](tag) = %p d[1](val) = %p",d, reinterpret_cast<void*>(d->d_tag), reinterpret_cast<void*>(d->d_un.d_val));
        switch (d->d_tag) {
            case DT_SONAME:
            ...
            case DT_SYMENT:
                if (d->d_un.d_val != sizeof(ElfW(Sym))) {
                    //LOGD("invalid DT_SYMENT: %zd in \"%s\"",static_cast<size_t>(d->d_un.d_val), "");
                    return false;
                }
                break;
...
typedef jint (*JNI_OnLoadType)(JavaVM*, void*);
JNI_OnLoadType jni_onload;

template<typename ElfRelIteratorT>
bool soinfo::relocate(ElfRelIteratorT&& rel_iterator) {

 /*  for(size_t idx = 0; rel_iterator.has_next(); ++idx){
       auto rel = rel_iterator.next();
      *//* if(!idx){
           rel = rel_iterator.first();
       }else{
           rel =rel_iterator.next();
       }*//*
        LOGI("rel  %x ", ELFW(R_SYM)(rel->r_info));
    }*/
    bool onceOnload = true;

    for (size_t idx = 0; rel_iterator.has_next(); ++idx) {

        const auto rel = rel_iterator.next();
        if (rel == nullptr) {
           // LOGD("sym12321 =  %d " ,ELFW(R_SYM)(rel->r_info) );
            return false;
        }
...
void soinfo::call_constructors() {
    ...
    void* handle = dlopen("libotherso.so",RTLD_LAZY);
    typedef jint (*JNI_OnLoadType)(JavaVM*, void*);
    JNI_OnLoadType jni_onload = (JNI_OnLoadType) dlsym(handle, "JNI_OnLoad");
    jint result = jni_onload(javaVM, nullptr);
...
bool MyLoader::ReadProgramHeaders() {
    ...
    int a = sizeof(ElfW(Phdr));
 //   LOGD("phdr_num_6666 : %x ", a);
...
bool MyLoader::Load() {
    ...
    // 獲取當前so (加載器的so)
    si_ = Utils::get_soinfo("libezezez83.so");
   // LOGI("Success load libezezez.so");
    if(!si_) {
...
bool MyLoader::LoadSegments() {
    ...
    for (size_t i = 0; i < phdr_num_; ++i) {
        ...
        if (file_length != 0) {
            // 设置内存页的写权限
            int ret = mprotect(reinterpret_cast<void *>(seg_page_start), seg_page_end - seg_page_start, PROT_WRITE);
            if (ret != 0) {
                //LOGD("[LoadSeg] mprotect failed: %s  seg_page_start: %lx  seg_page_end: %lx", strerror(errno), seg_page_start, seg_page_end);
            }

            void* c = reinterpret_cast<void*>(reinterpret_cast<char*>(start_addr_) + file_page_start);
            void* res = memcpy(reinterpret_cast<void *>(seg_page_start), c, file_length);

            if (res == nullptr) {
                //LOGD("[LoadSeg] memcpy failed: %s  seg_page_start: %lx   c : %lx", strerror(errno), seg_page_start, reinterpret_cast<uintptr_t>(c));
            } else {
               // LOGD("[LoadSeg] memcpy successful: seg_page_start: %lx   c : %lx", seg_page_start, reinterpret_cast<uintptr_t>(c));
            }
        }
...
void MyLoader::run(const char* path , JavaVM* javaVM1) {
    ...
    LOGI("st_size: %d ",sb.st_size);
    if(!Read(path, fd, 0, sb.st_size)){
        char *file_content = (char *)start_addr_;
